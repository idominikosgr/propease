...
import type { PropertySearchFilters } from '@/lib/supabase-service'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchParams.get('subareaIds')) {
      filters.subareaIds = searchParams.get('subareaIds')!.split(',').map(Number)
    }
    ...
    if (searchParams.get('subcategoryIds')) {
      filters.subcategoryIds = searchParams.get('subcategoryIds')!.split(',').map(Number)
    }
    if (searchParams.get('energyClassIds')) {
      filters.energyClassIds = searchParams.get('energyClassIds')!.split(',').map(Number)
    }
    ...
    if (filters.minSqrMeters) query = query.gte('sqr_meters', filters.minSqrMeters)
    if (filters.maxSqrMeters) query = query.lte('sqr_meters', filters.maxSqrMeters)
    if (filters.areaIds?.length) query = query.in('area_id', filters.areaIds)
    if (filters.subareaIds?.length) query = query.in('subarea_id', filters.subareaIds)
    if (filters.energyClassIds?.length) query = query.in('energy_class_id', filters.energyClassIds)
    ...
    if (filters.search) {
      query = query.or(
        `custom_code.ilike.%${filters.search}%,postal_code.ilike.%${filters.search}%`
      )
    }
          ...
          property.category_id === 1
            ...
            : property.category_id === 2
              ...
              : property.category_id === 3
                ...
                : 'Other',
        subcategory_name:
          property.subcategory_id === 1
            ? 'Apartment'
            : property.subcategory_id === 2
              ? 'House'
