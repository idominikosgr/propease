...
import { createIListClient, IListProperty } from "@/lib/ilist-api";
    ...
    switch (payload.event) {
      ...
      case "property.updated": {
        ...
        if (!propertyData) {
          ...
          const fetchedData = await ilistClient.fetchPropertyById(
            payload.property_id,
          );
          if (!fetchedData) {
            throw new Error(
              `Property ${payload.property_id} not found in iList`,
            );
          }
        ...
        const propertyId =
          await supabaseService.upsertPropertyFromIList(propertyData);
        ...
        await supabaseService.createSyncSession({
          ...
          api_responses: [
            { webhook_payload: payload, property_data: propertyData },
          ],
        });
      ...
      case "property.deleted": {
        ...
        const existingProperty = await supabaseService.getPropertyByIListId(
          payload.property_id,
        );
