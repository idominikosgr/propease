...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get total counts
    const [{ count: totalProperties }, { count: totalInquiries }, { count: totalSyncSessions }] =
      await Promise.all([
        supabaseService
          .getSupabase()
          .from('properties')
    ...
    const propertyMetrics = {
      ...
      averagePrice:
        properties && properties.length > 0
          ? Math.round(
              properties.reduce((acc: number, p: any) => acc + (p.price || 0), 0) /
                properties.length
            )
          : 0,
      priceRange: {
    ...
    const syncMetrics = {
      ...
      totalPropertiesProcessed:
        syncSessions?.reduce((acc: number, s: any) => acc + (s.total_properties || 0), 0) || 0,
      totalPropertiesAdded:
        syncSessions?.reduce((acc: number, s: any) => acc + (s.new_properties || 0), 0) || 0,
      totalPropertiesUpdated:
        syncSessions?.reduce((acc: number, s: any) => acc + (s.updated_properties || 0), 0) || 0,
    ...
    const inquiryMetrics = {
      ...
      conversionRate:
        inquiries && inquiries.length > 0
          ? Math.round(
              (inquiries.filter((i: any) => i.status === 'converted').length / inquiries.length) *
                100
            )
          : 0,
      byStatus: {
      ...
      averagePropertyPrice:
        inquiries && inquiries.length > 0
          ? Math.round(
              inquiries.reduce((acc: number, i: any) => acc + (i.properties?.price || 0), 0) /
                inquiries.length
            )
          : 0,
...
function generateDailyTimeSeries(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const dayData = {
      date: dateStr,
      properties: data.properties.filter((p: any) => p.created_at?.split('T')[0] === dateStr)
        .length,
      inquiries: data.inquiries.filter((i: any) => i.created_at?.split('T')[0] === dateStr).length,
      syncSessions: data.syncSessions.filter((s: any) => s.started_at?.split('T')[0] === dateStr)
        .length,
      conversions: data.inquiries.filter(
        (i: any) => i.created_at?.split('T')[0] === dateStr && i.status === 'converted'
      ).length,
    }
