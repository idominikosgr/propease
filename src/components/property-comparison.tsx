  ...
  const searchProperties = async (query: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const filtered = result.data.filter((prop: Property) => 
          !comparedProperties.some(compared => compared.id === prop.id)
        );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
            ...
            {searchResults.length > 0 && (
              <div className="mt-2 space-y-2 max-h-48 overflow-y-auto">
                {searchResults.map((property) => (
                  <div key={property.id} className="flex items-center justify-between p-2 border rounded bg-muted/50">
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-sm truncate">{property.title}</div>
                      <div className="text-xs text-muted-foreground">
                        {property.subarea_name}, {property.area_name} • {formatPrice(property.price)}
                      </div>
                    </div>
        ...
        <div className="overflow-x-auto">
          <div className="grid grid-cols-1 gap-4" style={{ minWidth: `${comparedProperties.length * 280}px` }}>
            {/* Property Cards */}
            <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${comparedProperties.length}, 1fr)` }}>
                  ...
                  <div className="relative h-48 bg-muted">
                    ...
                    <div className="absolute top-2 left-2 flex flex-col gap-1">
                      ...
                      {property.golden_visa_eligible && (
                        <Badge variant="default" className="bg-yellow-600/90 backdrop-blur-sm text-xs">
