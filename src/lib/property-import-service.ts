...
import type { IListProperty } from '@/lib/ilist-api'
import type {
  ...
  try {
    for (let i = 0; i < csvData.length; i++) {
      ...
      try {
        ...
        if (i === 0) {
          console.log('Transformed property sample:', JSON.stringify(ilistProperty, null, 2))
        }
        ...
        console.log(`Property ${i + 1} inserted successfully with ID:`, insertedId)
        result.success++
    ...
    if (sessionId) {
      await supabase
        ...
        .update({
          ...
          error_details:
            result.errorDetails.length > 0 ? ({ errors: result.errorDetails } as any) : null,
        })
        .eq('id', sessionId)
    }
    ...
    if (result.success > 0) {
      console.log(`Import completed successfully: ${result.success} properties imported`)
      console.log(
 ...
 */
function transformCsvToIListProperty(row: any, mapping: PropertyImportMapping): IListProperty {
  ...
  const partner =
    mapping.partner_name || mapping.partner_email || mapping.partner_phone
      ? {
          ...
          Lastname: row[mapping.partner_name!]?.split(' ').slice(1).join(' ') || '',
  ...
  const ilistProperty: IListProperty = {
    Id: ilistId,
    Category_ID: mapping.property_type ? parseInt(row[mapping.property_type]) || 1 : 1,
    ...
    Price: mapping.price ? parseFloat(String(row[mapping.price]).replace(/[^0-9.-]/g, '')) || 0 : 0,
    ...
    EnergyClass_ID: mapping.energy_class_id ? parseInt(row[mapping.energy_class_id]) : undefined,
    BuildingYear: mapping.building_year ? parseInt(row[mapping.building_year]) : undefined,
...
function validateIListProperty(property: IListProperty): string[] {
  ...
  if (!hasTitle) {
    console.warn('Property has no title characteristic, but allowing import for testing')
  }
