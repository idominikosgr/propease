...
import type { IListProperty, IListImage, IListCharacteristic, IListPartner } from './ilist-api'
...
export class SupabaseService {
  ...
  async upsertPropertyFromIList(ilistProperty: IListProperty): Promise<string> {
    const { data, error } = await this.supabase.rpc('upsert_property_from_ilist', {
    ...
    await Promise.all([
      ...
      this.upsertPropertyCharacteristics(propertyId, ilistProperty.Characteristics || []),
      this.upsertPropertyPartner(propertyId, ilistProperty.Partner),
      this.upsertPropertyDistances(propertyId, ilistProperty.DistanceFrom || []),
   ...
   */
  private async upsertPropertyImages(propertyId: string, images: IListImage[]): Promise<void> {
    if (!images.length) return

    // Delete existing images
    await this.supabase.from('property_images').delete().eq('property_id', propertyId)
    ...
    const { error } = await this.supabase.from('property_images').insert(imagesToInsert)
  ...
  private async upsertPropertyCharacteristics(
    ...
    await this.supabase.from('property_characteristics').delete().eq('property_id', propertyId)
   ...
   */
  private async upsertPropertyPartner(propertyId: string, partner?: IListPartner): Promise<void> {
    // Delete existing partner
    await this.supabase.from('property_partners').delete().eq('property_id', propertyId)
   ...
   */
  private async upsertPropertyDistances(propertyId: string, distances: any[]): Promise<void> {
    // Delete existing distances
    await this.supabase.from('property_distances').delete().eq('property_id', propertyId)
    ...
    const { error } = await this.supabase.from('property_distances').insert(distancesToInsert)
   ...
   */
  private async upsertPropertyParkings(propertyId: string, parkings: any[]): Promise<void> {
    // Delete existing parkings
    await this.supabase.from('property_parkings').delete().eq('property_id', propertyId)
    ...
    const { error } = await this.supabase.from('property_parkings').insert(parkingsToInsert)
   ...
   */
  private async upsertPropertyBasements(propertyId: string, basements: any[]): Promise<void> {
    // Delete existing basements
    await this.supabase.from('property_basements').delete().eq('property_id', propertyId)
    ...
    const { error } = await this.supabase.from('property_basements').insert(basementsToInsert)
   ...
   */
  private async upsertPropertyFlags(propertyId: string, flags: any[]): Promise<void> {
    // Delete existing flags
    await this.supabase.from('property_flags').delete().eq('property_id', propertyId)
    ...
    const { error } = await this.supabase.from('property_flags').insert(flagsToInsert)
   ...
   */
  async searchProperties(filters: PropertySearchFilters = {}): Promise<PropertyWithDetails[]> {
   ...
   */
  async getPropertyByIListId(ilistId: number): Promise<PropertyWithDetails | null> {
    const { data, error } = await this.supabase
      .from('properties')
      .select(
        `
   ...
   * Create sync session
   */
  async createSyncSession(syncData: Partial<IListSyncSession>): Promise<string> {
    const { data, error } = await this.supabase
   ...
   * Update sync session
   */
  async updateSyncSession(id: string, updates: Partial<IListSyncSession>): Promise<void> {
    const { error } = await this.supabase.from('ilist_sync_sessions').update(updates).eq('id', id)
   ...
   */
  async getPropertyInquiries(propertyId?: string, status?: string): Promise<PropertyInquiry[]> {
