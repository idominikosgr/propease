...
export class IListApiClient {
  ...
  async fetchLookupData(
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/lookups/${lookupType}`, {
        method: 'GET',
        headers: this.createHeaders({
      ...
      console.error(`Error fetching ${lookupType} lookup data from iList:`, error)
      throw error
    }
  }
   ...
   */
  async fetchAllLookups(languageId = 4): Promise<Record<string, IListLookup[]>> {
    const lookupTypes = [
      'PropertyAmenities',
      'PropertySecurity',
      'HeatingType',
    ...
    for (const lookupType of lookupTypes) {
      try {
        lookups[lookupType] = await this.fetchLookupData(lookupType, languageId)
   ...
   */
  async incrementalSync(lastSyncDate: Date): Promise<IListApiResponse<IListProperty[]>> {
   ...
   */
  async syncDeletedProperties(lastSyncDate?: Date): Promise<IListApiResponse<IListProperty[]>> {
    const params: IListSyncParams = {
 ...
 */
export function extractGreekText(characteristics: IListCharacteristic[], titleKey: string): string {
  const characteristic = characteristics.find((c) => c.Title === titleKey && c.Language_Id === 4)
...
export function getPrimaryImageUrl(property: IListProperty): string {
  ...
  const sortedImages = [...property.Images].sort((a, b) => a.OrderNum - b.OrderNum)
